% ==================================================================================================
% COMANDI DA RIDEFINIRE
% ==================================================================================================

% Nome/Versione/Data del documento
\newcommand{\documentName}{ERRORE}
\newcommand{\documentVersion}{ERRORE}
\newcommand{\documentDate}{ERRORE}

% Editori del documento
\newcommand{\documentEditors}{ERRORE}

% Verificatore del documento
\newcommand{\documentVerifiers}{ERRORE}

% Approvatore del documento
\newcommand{\documentApprovers}{ERRORE}

% Uso del documento
\newcommand{\documentUsage}{ERRORE}

% Destinatari del documento
\newcommand{\documentAddressee}{ERRORE}

% Sommario del documento
\newcommand{\documentSummary}{ERRORE}

% Vesione dei documenti
\newcommand{\docVersionGlo}{\textit{v2.0.0}} % Glossario
\newcommand{\docVersionNdP}{\textit{v3.0.0}} % Norme di Progetto
\newcommand{\docVersionSdC}{\textit{v1.0.0}} % Studio dei Capitolati
\newcommand{\docVersionPdP}{\textit{v3.0.0}} % Piano di Progetto
\newcommand{\docVersionPdQ}{\textit{v3.0.0}} % Piano di Qualifica
\newcommand{\docVersionST}{\textit{v1.0.0}} % Specifica Tecnica
\newcommand{\docVersionDdP}{\textit{v1.0.0}} % Definizione di Prodotto
\newcommand{\docVersionAdR}{\textit{v3.0.0}} % Analisi dei Requisiti
\newcommand{\docVersionMU}{\textit{v2.0.0}} % Manuale Utente
\newcommand{\docVersionMS}{\textit{v1.0.0}} % Manuale Sviluppatore

% ==================================================================================================
% COMANDI DA NON RIDEFINIRE
% ==================================================================================================

% Stile font
\newcommand{\glo}{\ap{\textbf{\tiny G}}} % Testo glossario

% Nome del progetto
\newcommand{\projectName}{Piattaforma di Localizzazione Testi}
\newcommand{\groupEmail}{\textit{\href{mailto:submarines.g4@gmail.com}{submarines.g4@gmail.com}}}
\newcommand{\groupName}{\textit{Submarines}}

% Componenti del gruppo
\newcommand{\team}{Michael Amista', Matteo Cescon, Loris Corradin, Niccolò Fasolo, Andrea Longo, Davide Marzaro, Samuel Scarabottolo}

% Ruoli del progetto
% Maiuscolo
\newcommand{\roleProjectManager}{\textit{Responsabile}}
\newcommand{\roleAdministrator}{\textit{Amministratore}}
\newcommand{\roleAnalyst}{\textit{Analista}}
\newcommand{\roleProgrammer}{\textit{Programmatore}}
\newcommand{\roleDesigner}{\textit{Progettista}}
\newcommand{\roleVerifier}{\textit{Verificatore}}

% Minuscolo
\newcommand{\roleProjectManagerLow}{\textit{responsabile}}
\newcommand{\roleAdministratorLow}{\textit{amministratore}}
\newcommand{\roleAnalystLow}{\textit{analista}}
\newcommand{\roleProgrammerLow}{\textit{programmatore}}
\newcommand{\roleDesignerLow}{\textit{progettista}}
\newcommand{\roleVerifierLow}{\textit{verificatore}}

% Referenti e committenti
\newcommand{\proposerName}{Michele Massaro - Zero12 s.r.l.}
\newcommand{\commitNameM}{Prof. Tullio Vardanega}
\newcommand{\commitNameS}{Prof. Riccardo Cardin}

% Nome dei documenti
% Maiuscolo
\newcommand{\docNameGlo}{\textit{Glossario}} % Glossario
\newcommand{\docNameNdP}{\textit{Norme di progetto}} % Norme di Progetto
\newcommand{\docNameSdC}{\textit{Studio dei capitolati}} % Studio dei Capitolati
\newcommand{\docNameST}{\textit{Specifica tecnica}} % Specifica Tecnica
\newcommand{\docNameDdP}{\textit{Definizione di prodotto}} % Definizione di Prodotto
\newcommand{\docNamePdP}{\textit{Piano di progetto}} % Piano di Progetto
\newcommand{\docNamePdQ}{\textit{Piano di qualifica}} % Piano di Qualifica
\newcommand{\docNamePoC}{\textit{Proof of Concept}} % Proof of Concept
\newcommand{\docNameAdR}{\textit{Analisi dei requisiti}} % Analisi dei Requisiti
\newcommand{\docNameMU}{\textit{Manuale utente}} % Manuale Utente
\newcommand{\docNameMS}{\textit{Manuale sviluppatore}} % Manuale Sviluppatore
\newcommand{\docNameI}{\textit{Impegni}} % Impegni
\newcommand{\docNameV}{\textit{Verbale}} % Verbale

% Minuscolo
\newcommand{\docNameGloLow}{\textit{glossario}} % Glossario
\newcommand{\docNameNdPLow}{\textit{norme di progetto}} % Norme di Progetto
\newcommand{\docNameSdCLow}{\textit{studio dei capitolati}} % Studio dei Capitolati
\newcommand{\docNameSTLow}{\textit{specifica tecnica}} % Specifica Tecnica
\newcommand{\docNameDdPLow}{\textit{definizione di prodotto}} % Definizione di Prodotto
\newcommand{\docNamePdPLow}{\textit{piano di progetto}} % Piano di Progetto
\newcommand{\docNamePdQLow}{\textit{piano di qualifica}} % Piano di Qualifica
\newcommand{\docNameAdRLow}{\textit{analisi dei requisiti}} % Analisi dei Requisiti
\newcommand{\docNameMULow}{\textit{manuale utente}} % Manuale Utente
\newcommand{\docNameMSLow}{\textit{manuale sviluppatore}} % Manuale Sviluppatore
\newcommand{\docNameILow}{\textit{impegni}} % Impegni
\newcommand{\docNameVLow}{\textit{verbale}} % Verbale

% Nome e versione dei documenti
\newcommand{\docNameVersionGlo}{\docNameGlo{} \docVersionGlo} % Glossario
\newcommand{\docNameVersionNdP}{\docNameNdP{} \docVersionNdP} % Norme di Progetto
\newcommand{\docNameVersionSdC}{\docNameSdC{} \docVersionSdC} % Studio di Fattibilità
\newcommand{\docNameVersionPdP}{\docNamePdP{} \docVersionPdP} % Piano di Progetto
\newcommand{\docNameVersionPdQ}{\docNamePdQ{} \docVersionPdQ} % Piano di Qualifica
\newcommand{\docNameVersionAdR}{\docNameAdR{} \docVersionAdR} % Analisi dei Requisiti
\newcommand{\docNameVersionMU}{\docNameMU{} \docVersionMU} % Manuale Utente
\newcommand{\docNameVersionMS}{\docNameMS{} \docVersionMS} % Manuale Sviluppatore

% Nome delle revisioni
\newcommand{\RTB}{\textit{Requirements and Technology Baseline}}
\newcommand{\PB}{\textit{Product Baseline}}
\newcommand{\CA}{\textit{Customer Acceptance}}

% Descrizione del glossario
\newcommand{\gloDesc}{Per evitare di generare dubbi o ambiguità, è stato creato un documento denominato \docNameVersionGlo{}, che contiene tutti i termini per i quali il gruppo ha ritenuto
adeguato fornire una spiegazione. All'interno di questa documentazione, tali termini sono contrassegnati da una \glo{} all'apice della parola.}

%parole pedice
\newcommand{\mysubscript}[1]{\raisebox{-0.34ex}{\scriptsize#1}}